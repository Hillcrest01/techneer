...
def change_password(customer_id):
    ...
    if form.validate_on_submit:
        ...
        #you can use the form.field.data instead of request.form.get('field')
        customer = Customer.query.first()

        if check_password_hash(customer.password_hash , password):
            if new_password == confirm_new_password:
                customer.password_hash = confirm_new_password
                db.session.commit()
                flash('password updated successfully')
                return redirect(f'profile/ {customer.id}')

            else:
                flash('new password does not match the confirm new password')
